...
import { query, create, remove, update, sort } from '../../services/sys/menu'
...
export default {
  ...
  effects: {
    ...
    * query ({ payload }, { call, put }) {
      ...
      if (data) {
        yield put({
          type: 'querySuccess',
          payload: {
            treeData: data.menuTree,
            menuPermissionTreeData: data.menuPermissionTree,
          },
        })
      }
    },
    ...
    * delete (action, { call, put, select }) {
      const id = yield select(({ menu }) => menu.latestItemId)
    ...
    * update ({ payload }, { select, call, put }) {
      const id = yield select(({ menu }) => menu.currItem.id)
  ...
  reducers: {
    ...
    querySuccess (state, action) {
      const { treeData, menuPermissionTreeData } = action.payload
      return {
        ...state,
        treeData,
        menuPermissionTreeData,
    ...
    onAdd (state, action) {
      ...
      const { latestItemId } = state
      let currItem = { mpid: isRoot ? null : latestItemId }
      return {
        ...state,
        currItem,
        formVisible: true,
